...
func main() {
	...
	DoTestRentableCountByRentableType(ctx)
	//	DoTestRentable(ctx)
}
...
func ReportTextOutput(tbl *gotable.Table) {
	...
	fname := "RentableCountByRentableTypeReport_test.txt"
	...
	if nil != err {
		...
		log.Fatalf("%s: RentableCountByRentableTypeReport_test: Error creating file: %s\n", fname, err.Error())
	}

	if err := tbl.TextprintTable(f); err != nil {
		log.Fatalf("RentableCountByRentableTypeReport_test: Error creating TEXT output: %s\n", err.Error())
	}
	// close file after operation
	f.Close()

	// now compare what we have to the known-good output
	b, _ := ioutil.ReadFile("./testdata/RentableCountByRentableTypeReport_test.txt")
	sb, _ := ioutil.ReadFile("./RentableCountByRentableTypeReport_test.txt")

	if len(b) != len(sb) {
		// fmt.Printf("RentableCountByRentableTypeReport_test: Expected len = %d,  found len = %d\n", len(b), len(sb))
		log.Fatalf("RentableCountByRentableTypeReport_test: Expected len = %d,  found len = %d\n", len(b), len(sb))
	}
	if len(sb) > 0 && len(b) > 0 {
		for i := 0; i < len(b); i++ {
			if i < len(sb) && sb[i] != b[i] {
				log.Fatalf("RentableCountByRentableTypeReport_test: micompare at character %d, expected %x (%c), found %x (%c)\n", i, b[i], b[i], sb[i], sb[i])
				// fmt.Printf("RentableCountByRentableTypeReport_test: micompare at character %d, expected %x (%c), found %x (%c)\n", i, b[i], b[i], sb[i], sb[i])
				break
			}
		}
	}
...
func ReportCSVOutput(tbl *gotable.Table) {
	...
	f, err := os.Create(fname)
	if nil != err {
		log.Fatalf("RentableCountByRentableTypeReport_test: Error creating file %s: %s\n", fname, err.Error())
		// fmt.Printf("smoke_test: Error creating file: %s\n", err.Error())
	}

	if err := tbl.CSVprintTable(f); err != nil {
		log.Fatalf("RentableCountByRentableTypeReport_test: Error creating CSV output: %s\n", err.Error())
	}
	// close file after operation
	f.Close()

	// now compare what we have to the known-good output
	b, _ := ioutil.ReadFile("./testdata/RentableCountByRentableTypeReport_test.csv")
	sb, _ := ioutil.ReadFile("./RentableCountByRentableTypeReport_test.csv")
...
func ReportHTMLOutput(tbl *gotable.Table) {
	...
	if err := tbl.HTMLprintTable(f); err != nil {
		log.Fatalf("RentableCountByRentableTypeReport_test: Error creating HTML output: %s\n", err.Error())
	}
	// close file after operation
	f.Close()

	// now compare what we have to the known-good output
	b, _ := ioutil.ReadFile("./testdata/RentableCountByRentableTypeReport_test.html")
	sb, _ := ioutil.ReadFile("./RentableCountByRentableTypeReport_test.html")
...
func ReportPDFOutput(tbl *gotable.Table) {
	...
	if err := tbl.PDFprintTable(f, pdfProps); err != nil {
		log.Fatalf("RentableCountByRentableTypeReport_test: Error creating PDF output: %s\n", err.Error())
	}
	// close file after operation
	f.Close()

	// now compare what we have to the known-good output
	b, _ := ioutil.ReadFile("./testdata/RentableCountByRentableTypeReport_test.pdf")
	sb, _ := ioutil.ReadFile("./RentableCountByRentableTypeReport_test.pdf")

	if len(sb) == 0 {
		log.Fatalf("RentableCountByRentableTypeReport_test: Expected some content in PDF output file,  found len = 0")
	}
...
func DoTestRentableCountByRentableType(ctx context.Context) {
	...
	tmpT2 := time.Date(2018, 9, 1, 8, 0, 0, 0, time.UTC)
	...
	//ri.Xbiz = rlib.Designation

	tlb := rrpt.RentableCountByRentableTypeReportTable(ctx, &ri)
	...
	/*for i := 0; i < len(tlb.Row); i++ {
		for j := 0; j < len(tlb.Row[i].Col); j++ {
			switch tlb.Row[i].Col[j].Type {
			case 0:
				fmt.Printf("%d", tlb.Row[i].Col[j].Ival)
			case 1:
				fmt.Printf("%f", tlb.Row[i].Col[j].Fval)
			case 2:
				fmt.Printf(tlb.Row[i].Col[j].Sval)
			}
		}
	}*/
}
