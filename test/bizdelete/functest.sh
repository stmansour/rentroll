...
mysqlverify "o"  "${RRCTX}"		"Users"	                    	"select TCID,Points,DateofBirth,EmergencyContactName,EmergencyContactAddress,EmergencyContactTelephone,EmergencyContactEmail,AlternateEmailAddress,EligibleFutureUser,Industry,SourceSLSID from User;"
mysqlverify "p"  "${RRCTX}"		"Payors"	                    "select TCID,CreditLimit,TaxpayorID,LastModBy from Payor;"
mysqlverify "q"  "${RRCTX}"		"Prospects"	            		"select TCID,CompanyAddress,CompanyCity,CompanyState,CompanyPostalCode,CompanyEmail,CompanyPhone,Occupation,ThirdPartySource,LastModBy from Prospect;"
...
mysqlverify "d1" "${RRCTX}"		"Assessments"	            	"select ASMID,BID,RID,RAID,Amount,Start,Stop,RentCycle,ProrationCycle,AcctRule,Comment,LastModBy from Assessments;"
