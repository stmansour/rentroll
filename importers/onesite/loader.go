...
4. frequency (optional) (rent cycle frequency) (0: one time only | 1: secondly | 2: minutely | 3: hourly | 4: daily | 5: weekly | 6: monthly | 7: quarterly | 8: yearly )
...
CSVHandler accepts csv file path, testmode and user supplied values. It initializes config first for onesite loader via init() call. All user passed values should be validated in it first. After that it calls main function `LoadOneSiteCSV` and then creates a report based on response of `LoadOneSiteCSV` call.

LoadOneSiteCSV first loads field mapping defined from mapper.json in struct. Then after loading csv data, in first loop it skips rows that are meant for onesite data to import and then performs data validation on onesite csv data. If there is any error in validation then it just simply returns. Before going to second iteration loop it performs some necessary operation including get file pointers, writer pointers, declaring struct to avoid duplicate data, declaring struct to trace the data in accordance of input csv, declaring count variables for each db type, declare customAttribRefData struct. Then it loads data into temporary files in favor of onesite rcsv loader to import the data via calls of rcsv routines. After data import has been done, it will dump customAttribRefs in rentroll in a manual way rather than importing via temp csv file. At last, after all things done, it clears out all temp files from `temp_CSVs` dir and returns the response.
*/
