...
type assessment struct {
	...
	Start     time.Time
	Stop      time.Time
	Frequency int  // 0=none, 1=daily, 2=weekly, 3=monthly, 4=quarterly, 5=yearly
}
...
func runRecurTest(t *testing.T, name string, rrcase []rrtest, dt1, dt2 string) {
	...
	for i := 0; i < len(rrcase); i++ {
		...
		errcheck(err)
		var a = assessment{0, 0, 0, rrcase[i].amt, t1, t2, rrcase[i].freq}
		ap := &a
		m := ap.getRecurrences(&dtstart, &dtstop)
		for j := 0; j < len(m); j++ {
