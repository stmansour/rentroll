...
func buildPreparedStatements() {
	...
	Errcheck(err)
	RRdb.Prepstmt.GetUnitSpecialties, err = RRdb.dbrr.Prepare("SELECT USPID FROM rentablespecialties where BID=? and UNITID=?")
	Errcheck(err)
	RRdb.Prepstmt.GetRentableSpecialty, err = RRdb.dbrr.Prepare("SELECT USPID,BID,Name,Fee,Description FROM unitspecialtytypes where USPID=?")
	Errcheck(err)
