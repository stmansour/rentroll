...
func buildPreparedStatements() {
	...
	Errcheck(err)
	RRdb.Prepstmt.GetRentable, err = RRdb.dbrr.Prepare("SELECT RID,LID,RTID,BID,Name,Assignment,Report,DefaultOccType,OccType,LastModTime,LastModBy FROM rentable where RID=?")
	Errcheck(err)
	...
	Errcheck(err)
	RRdb.Prepstmt.GetAllBusinessSpecialtyTypes, err = RRdb.dbrr.Prepare("SELECT USPID,BID,Name,Fee,Description FROM rentablespecialtytypes WHERE BID=?")
	Errcheck(err)
