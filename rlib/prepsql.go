...
func buildPreparedStatements() {
	...
	//  AR
	//===============================
	flds = "ARID,BID,Name,ARType,DebitLID,CreditLID,Description,RARequired,DtStart,DtStop,FLAGS,DefaultAmount,DefaultRentCycle,DefaultProrationCycle,CreateTS,CreateBy,LastModTime,LastModBy"
	...
	Errcheck(err)
	RRdb.Prepstmt.GetAllAssessmentsByBusiness, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM Assessments WHERE BID=? AND (PASMID=0 OR RentCycle=0) AND Start<? AND Stop>=? ORDER By Start ASC, Amount DESC")
	Errcheck(err)
	RRdb.Prepstmt.GetRecurringAssessmentsByBusiness, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM Assessments WHERE BID=? AND PASMID=0 AND RentCycle>0 AND Start<? AND Stop>=? ORDER By Start ASC, Amount DESC")
	Errcheck(err)
	RRdb.Prepstmt.GetEpochAssessmentsByRentalAgreement, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM Assessments WHERE RAID=? AND (PASMID=0 or RentCycle=0) ORDER By Start ASC, Amount DESC")
	Errcheck(err)
	RRdb.Prepstmt.GetAllSingleInstanceAssessments, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM Assessments WHERE BID=? AND (PASMID!=0 OR RentCycle=0) AND Start<? AND Stop>=? ORDER By Start ASC, Amount DESC")
	Errcheck(err)
	RRdb.Prepstmt.GetAssessmentsByRAIDRID, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM Assessments WHERE PASMID = 0 AND BID=? AND RAID=? AND RID=? ORDER By Start ASC, Amount DESC")
	Errcheck(err)
...
WHERE RAID=? AND RID=? AND Start >= ? AND Stop < ? AND RentCycle=0 ORDER By Assessments.Start ASC, Assessments.Amount DESC`)
	...
	Errcheck(err)
	RRdb.Prepstmt.GetReceiptsInDateRange, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM Receipt WHERE BID=? AND Dt >= ? AND Dt < ? ORDER BY Dt ASC, Amount ASC")
	Errcheck(err)

	//  FLAGS bits 0-1:  0 unallocated, 1 = partially allocated, 2 = fully allocated,  bit 2: voided entry
	RRdb.Prepstmt.GetUnallocatedReceipts, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM Receipt WHERE BID=? AND (FLAGS & 7)<2 ORDER BY Dt ASC, Amount ASC")
	Errcheck(err)
	RRdb.Prepstmt.GetUnallocatedReceiptsByPayor, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM Receipt WHERE BID=? AND TCID=? AND (FLAGS & 3)<2 AND 0=(FLAGS & 4) ORDER BY Dt ASC, Amount ASC")
	Errcheck(err)
	...
	//  Rentable
	//===============================
	flds = "RID,BID,PRID,RentableName,AssignmentTime,MRStatus,DtMRStart,Comment,CreateTS,CreateBy,LastModTime,LastModBy"
	...
	//  Rental Agreement
	//===============================
	flds = "RAID,PRAID,ORIGIN,RATID,BID,NLID,DocumentDate,AgreementStart,AgreementStop,PossessionStart,PossessionStop,RentStart,RentStop,RentCycleEpoch,UnspecifiedAdults,UnspecifiedChildren,Renewal,SpecialProvisions,LeaseType,ExpenseAdjustmentType,ExpensesStop,ExpenseStopCalculation,BaseYearEnd,ExpenseAdjustment,EstimatedCharges,RateChange,CSAgent,NextRateChange,PermittedUses,ExclusiveUses,ExtensionOption,ExtensionOptionNotice,ExpansionOption,ExpansionOptionNotice,RightOfFirstRefusal,DesiredUsageStartDate,RentableTypePreference,FLAGS,ApplicationReadyUID,ApplicationReadyDate,Approver1,DecisionDate1,DeclineReason1,Approver2,DecisionDate2,DeclineReason2,MoveInUID,MoveInDate,ActiveUID,ActiveDate,Outcome,NoticeToMoveUID,NoticeToMoveDate,NoticeToMoveReported,TerminatorUID,TerminationDate,TerminationStarted,LeaseTerminationReason,CreateTS,CreateBy,LastModTime,LastModBy"
	...
	Errcheck(err)
	RRdb.Prepstmt.GetRentalAgreementChain, err = RRdb.Dbrr.Prepare("SELECT " + flds + " from RentalAgreement WHERE ORIGIN=? OR RAID=? ORDER BY AgreementStart ASC")
	Errcheck(err)
	...
	RRdb.Prepstmt.GetRentalAgreementPayorsByRAID, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM RentalAgreementPayors WHERE RAID=?")
	Errcheck(err)
	RRdb.Prepstmt.GetRentalAgreementPayorsInRange, err = RRdb.Dbrr.Prepare("SELECT " + flds + " FROM RentalAgreementPayors WHERE RAID=? AND ?<DtStop AND ?>DtStart ORDER BY DtStart ASC, TCID ASC")
	Errcheck(err)
	...
	//  RentableUseStatus
	//===============================
	flds = "RSID,RID,BID,DtStart,DtStop,Comment,UseStatus,CreateTS,CreateBy,LastModTime,LastModBy"
	...
	//  RentableLeaseStatus
	//===============================
	flds = "RLID,RID,BID,RAID,DtStart,DtStop,LeaseStatus,Comment,FirstName,LastName,Email,Phone,Address,Address2,City,State,PostalCode,Country,CCName,CCType,CCNumber,CCExpMonth,ConfirmationCode,CreateTS,CreateBy,LastModTime,LastModBy"
