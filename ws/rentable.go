...
var rentablesGridFieldsMap = map[string][]string{
	...
	"LeaseStatus":          {"RL.LeaseStatus"}, //ADD BY LINA
	"RARID":                {"RAR.RARID"},
	"RAID":                 {"RAR.RAID"},
...
func SvcSearchHandlerRentables(w http.ResponseWriter, r *http.Request, d *ServiceData) {
	...
	order := "R.RID ASC, RAR.RARID DESC, RTR.RTRID DESC, RS.RSID, RL.RLID DESC" // default ORDER, ADD RL.RLID BY LINA
	...
	LEFT JOIN (
		SELECT LeaseStatus,RID,RLID
		FROM RentableLeaseStatus
		WHERE DtStop > "{{.searchStart}}" AND DtStart <= "{{.searchStop}}" AND BID={{.BID}}
		GROUP BY RLID
		ORDER BY RLID DESC
	) AS RL ON RL.RID=R.RID
    WHERE {{.WhereClause}}
    GROUP BY R.RID
	...
	rlib.Console(rentablesQuery)
	rlib.Console(qc["SelectClause"])
...
// rsLeaseGridRowScan scans a result from sql row and dump it in a struct for rentableStatusGridRec, add on 01/23/2019
