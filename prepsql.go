...
func buildPreparedStatements() {
	...
	rlib.Errcheck(err)
	App.prepstmt.getRentableType, err = App.dbrr.Prepare("SELECT RTID,PRID,Name,Amount,Frequency,Proration,LastModTime,LastModBy FROM rentabletypes where RTID=?")
	rlib.Errcheck(err)
	App.prepstmt.getUnitType, err = App.dbrr.Prepare("SELECT UTID,PRID,Style,Name,SqFt,MarketRate,Frequency,Proration,LastModTime,LastModBy FROM unittypes where UTID=?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getUnitAssessments, err = App.dbrr.Prepare("SELECT ASMID,RID,UNITID,ASMTID,RAID,Amount,Start,Stop,Frequency,ProrationMethod,LastModTime,LastModBy FROM assessments WHERE UNITID=? and Stop >= ? and Start < ?")
	rlib.Errcheck(err)
	App.prepstmt.getAllRentableAssessments, err = App.dbrr.Prepare("SELECT ASMID,RID,UNITID,ASMTID,RAID,Amount,Start,Stop,Frequency,ProrationMethod,LastModTime,LastModBy FROM assessments WHERE RID=? and Stop >= ? and Start < ?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getUnitRentalAgreements, err = App.dbrr.Prepare("SELECT RAID,RATID,PRID,RID,UNITID,PID,PrimaryTenant,RentalStart,RentalStop,Renewal,SpecialProvisions,LastModTime,LastModBy from rentalagreement where unitid=? and RentalStop > ? and RentalStart < ?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getAllPropertyRentableTypes, err = App.dbrr.Prepare("SELECT RTID,PRID,Name,Amount,Frequency,Proration,LastModTime,LastModBy FROM rentabletypes WHERE PRID=?")
	rlib.Errcheck(err)
	App.prepstmt.getAllPropertyUnitTypes, err = App.dbrr.Prepare("SELECT UTID,PRID,Style,Name,SqFt,MarketRate,Frequency,Proration,LastModTime,LastModBy FROM unittypes WHERE PRID=?")
	rlib.Errcheck(err)
