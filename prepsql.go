...
func buildPreparedStatements() {
	...
	rlib.Errcheck(err)
	App.prepstmt.getLedger, err = App.dbrr.Prepare("SELECT LID,BID,GLNumber,Dt,Status,Type,Amount FROM ledger where LID=?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getRentableType, err = App.dbrr.Prepare("SELECT RTID,BID,Name,Frequency,Proration,LastModTime,LastModBy FROM rentabletypes where RTID=?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getAllBusinessRentableTypes, err = App.dbrr.Prepare("SELECT RTID,BID,Name,Frequency,Proration,LastModTime,LastModBy FROM rentabletypes WHERE BID=?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getLedgerMarkerByGLNo, err = App.dbrr.Prepare("SELECT LMID,BID,GLNumber,State,Dt,Balance,DefaultAcct,Name FROM ledgermarker WHERE GLNumber=?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getDefaultCashLedgerMarker, err = App.dbrr.Prepare("SELECT LMID,BID,GLNumber,State,Dt,Balance,DefaultAcct,Name FROM ledgermarker where BID=? and DefaultAcct=1")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getJournalAllocations, err = App.dbrr.Prepare("SELECT JID,Amount,ASMID,AcctRule from journalallocation WHERE JID=?")
	rlib.Errcheck(err)
