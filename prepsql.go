...
func buildPreparedStatements() {
	...
	App.prepstmt.occAgrByBusiness, err = App.dbrr.Prepare("SELECT RAID,RATID,BID,RID,UNITID,PID,PrimaryTenant,RentalStart,RentalStop,Renewal,SpecialProvisions,LastModTime,LastModBy from rentalagreement where BID=?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getRentable, err = App.dbrr.Prepare("SELECT RID,LID,RTID,BID,PID,RAID,UNITID,Name,Assignment,Report,DefaultOccType,OccType,LastModTime,LastModBy FROM rentable where RID=?")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getAllRentablesByBusiness, err = App.dbrr.Prepare("SELECT RID,LID,RTID,BID,PID,RAID,UNITID,Name,Assignment,Report,LastModTime,LastModBy FROM rentable WHERE BID=?")
	rlib.Errcheck(err)
