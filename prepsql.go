...
func buildPreparedStatements() {
	...
	rlib.Errcheck(err)
	App.prepstmt.getLedgerMarkerByGLNoDateRange, err = App.dbrr.Prepare("SELECT LMID,BID,PID,GLNumber,Status,State,DtStart,DtStop,Balance,Type,Name FROM ledgermarker WHERE BID=? and GLNumber=? and DtStop>? and DtStart<? ORDER BY GLNumber ASC")
	rlib.Errcheck(err)
	...
	rlib.Errcheck(err)
	App.prepstmt.getLedgerMarkerInitList, err = App.dbrr.Prepare("SELECT LMID,BID,PID,GLNumber,Status,State,DtStart,DtStop,Balance,Type,Name from ledgermarker WHERE BID=? and State=3 ORDER BY GLNumber ASC")
	rlib.Errcheck(err)
