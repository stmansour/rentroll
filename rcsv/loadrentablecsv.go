...
func CreateRentables(sa []string, lineno int) (string, int) {
	...
	if len(des) > 0 {
		...
		if len(b1.Designation) == 0 {
			rs += fmt.Sprintf("%s: line %d - Business with bud %s does not exist\n", funcname, lineno, des)
	...
	if err != nil {
		...
		if !strings.Contains(s, "no rows") {
			rs += fmt.Sprintf("%s: lineno %d - error with rlib.GetRentableByName: %s\n", funcname, lineno, err.Error())
			return rs, CsvErrorSensitivity
		}
	}
	if r1.RID > 0 {
		rs += fmt.Sprintf("%s: lineno %d - Rentable with name \"%s\" already exists. Skipping. \n", funcname, lineno, r.Name)
		return rs, CsvErrorSensitivity
	}
	...
	if len(sa[2]) > 0 {
		...
		if err != nil || i < 0 || i > 2 {
			rs += fmt.Sprintf("%s: lineno %d - invalid AssignmentTime number: %s\n", funcname, lineno, sa[2])
			return rs, CsvErrorSensitivity
		}
	...
	if 0 == len(strings.TrimSpace(sa[4])) {
		rs += fmt.Sprintf("%s: lineno %d - rlib.RentableStatus value is required.\n",
			funcname, lineno)
		return rs, CsvErrorSensitivity
	}
	...
	for i := 0; i < len(st); i++ {  //spin through the 3-tuples
		...
		if err != nil || ix < rlib.RENTABLESTATUSONLINE || ix > rlib.RENTABLESTATUSLAST {
			rs += fmt.Sprintf("%s: lineno %d - invalid Status value: %s.  Must be in the range %d to %d\n",
	...
	if 0 == len(strings.TrimSpace(sa[5])) {
		rs += fmt.Sprintf("%s: lineno %d - rlib.Rentable RTID Ref value is required.\n",
			funcname, lineno)
		return rs, CsvErrorSensitivity
	}
	...
	for i := 0; i < len(st); i++ { // spin through the 3-tuples
		...
		if len(ss) != 3 {
			rs += fmt.Sprintf("%s: lineno %d - invalid RTID syntax. Each semi-colon separated field must have 3 values. Found %d in \"%s\"\n",
				funcname, lineno, len(ss), ss)
			return rs, CsvErrorSensitivity
		}
		...
		if err != nil {
			rs += fmt.Sprintf("%s: lineno %d - Could not load rentable type with style name: %s  -- error = %s\n",
	...
	if nil != err {
		rs += fmt.Sprintf("%s: lineno %d - error inserting rlib.Rentable = %v\n", funcname, lineno, err)
	}
	if rid > 0 {
		for i := 0; i < len(rul); i++ {
			rul[i].RID = rid
		...
		for i := 0; i < len(m); i++ {
			...
			if err != nil {
				rs += fmt.Sprintf("%s: lineno %d - error saving rlib.RentableStatus: %s\n", funcname, lineno, err.Error())
			}
		}
		for i := 0; i < len(n); i++ {
			n[i].RID = rid
...
func LoadRentablesCSV(fname string) string {
	rs := ""
	t := rlib.LoadCSV(fname)
	for i := 0; i < len(t); i++ {
		s, err := CreateRentables(t[i], i+1)
		rs += s
		if err > 0 {
			break
		}
